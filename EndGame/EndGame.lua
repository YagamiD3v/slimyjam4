...
function EndGame.drawButton(self)
...
function EndGame.newButton(posY, pText, pFunction)
  local y = posY
  --
  local w=Screen.w/3
  local h=Screen.h/10
  ...
  local cy = y + oy
  --
  local button = {x=x, y=y, w=w, h=h, ox=ox, oy=oy, cx=cx, cy=cy, drawButton=EndGame.drawButton, drawTextButton=EndGame.drawTextButton, draw=draw, action=pFunction}
  EndGame.newText(button, pText)
  --
